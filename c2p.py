...
class prims:
    ...
    def __sub__(self,other):
        return (abs((self.rho-other.rho)/(1e-20+self.rho))+
                abs((self.eps-other.eps)/(1e-20+self.eps))+
                abs((self.vel[0]-other.vel[0])/(1e-20+self.vel[0]))+
                abs((self.vel[1]-other.vel[1])/(1e-20+self.vel[1]))+
                abs((self.vel[2]-other.vel[2])/(1e-20+self.vel[2])))/5
class c2p:
    ...
    def invert(self, zeta=None):
        ...
        # Call brent on func
        if zeta is None:
            zeta = brentq(func,zm,zp,xtol=1e-15)
